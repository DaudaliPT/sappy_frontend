...
class ByUsComboBox extends Component {
  constructor(props) {
    super(props);
  ...
  loadProps(props) {
    ...
    if (this.props.getOptionsApiRoute) {
      ...
      this.serverRequest = axios
        ...
        .catch(error => {
          var msg = byUs.parseBackendError("Erro ao obter valores:", error);
...
function NameOptionRenderer({
  ...
  if (option.type && option.type === "group") {
    classNames.push("byus-option-group");
    ...
    classNames.push("byus-option");
    if (option === focusedOption) {
      classNames.push("byus-option-focused");
    }
    if (valueArray && valueArray.indexOf(option) >= 0) {
      classNames.push("byus-option-selected");
    }
