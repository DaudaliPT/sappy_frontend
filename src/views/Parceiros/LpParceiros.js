...
class Produtos extends Component {
  ...
  handleRowClick(e) {
    ...
    if (itemCode.indexOf("DRAFT") > -1) {
      this.setState({
        currentModal: <EditNewModal toggleModal={this.toggleModal} changeCardCode={itemCode} />
      });
    } else {
            ...
            <div className="row hidden-lg-down">
              ...
              <div
                ...
                onMouseEnter={e => {
                  ...
                  that.hoverTimeOutHandle = setTimeout(function() {
                    ...
                    that.hoverServerRequest = axios({ method: "get", url: `api/prod/${row.CardCode}/priceinfo` })
                      .then(result => {
                        let content = [];
                        result.data.forEach(row => {
                          content.push(
                            <tr>
                              ...
                              <td>
                                {sappy.format.price(row.Price)}
                              </td>
                              <td>
                                {sappy.format.date(row.U_apyUPDATED)}
                              </td>
                            </tr>
                          );
                        });
              ...
              <div
                ...
                onMouseEnter={e => {
                  ...
                  that.hoverTimeOutHandle = setTimeout(function() {
                    if (that.hoverServerRequest && that.hoverServerRequest.abort) that.hoverServerRequest.abort();
                    that.hoverServerRequest = axios({ method: "get", url: `api/prod/${row.CardCode}/stockinfo` })
                      .then(result => {
                        let content = [];
                        ...
                        result.data.forEach(popuprow => {
                          content.push(
                            <tr>
                              ...
                              <td>
                                {sappy.format.quantity(popuprow.OnHand, 0) + " " + row.InvntryUom}
                              </td>
                            </tr>
                          );
                        });
