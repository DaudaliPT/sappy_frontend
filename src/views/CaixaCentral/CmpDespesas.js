  ...
  render() {
    ...
    let renderRowPN = ({ row, index }) => {
      ...
      if (row.TransType === "46") {
        ...
        entidade = "Adiantamento a " + row.ContactName + (row.CounterRef ? ", " + row.CounterRef : "") + (row.Comments ? ", " + row.Comments : "");
      }
      ...
      let fixedActions = [
        {
          ...
          icon: showActions ? "icon wb-close animation-fade" : isAdiantamentoSelected ? "icon wb-more-vertical" : "icon wb-plus",
          onClick: e => {
      ...
      <div>
        <div className="row">
          <div className="col-12">
            <SearchPage ref={node => (this.pnComponent = node)} searchApiUrl={`/api/caixa/despesas`} renderRow={renderRowPN} renderRowHeight={50} />
          </div>
        </div>
