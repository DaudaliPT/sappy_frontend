...
class ModalDespesa extends Component {
          ...
          .then(result => {
            let options = result.data.map(row => {
              return {
                ...
                value: row.CreatedBy,
                label:
                  "Adiantamento a " + row.ContactName + " [" + sappy.format.amount(row.VALOR_PENDENTE) + "]" + (row.CounterRef ? ", " + row.CounterRef : "") + (row.Comments ? ", " + row.Comments : "")
              };
            });
  ...
  onFieldChange(changeInfo) {
    ...
    if (MeioDePagamentoEscolhido.VALOR_PENDENTE) {
      newStateValues.DiferrencaAdiantamento = sappy.format.amount(sappy.getNum(MeioDePagamentoEscolhido.VALOR_PENDENTE) - totalPagar);
    }
  ...
  render() {
    ...
    return (
      <Modal isOpen={true} className="modal-md modal-success">
        <ModalHeader toggle={this.props.toggleModal}>Registar despesa de caixa</ModalHeader>
        <ModalBody>
          <div className="container">
            <div className="row">
              <div className="col-9 pr-1">
                <ComboBox label="Fornecedor" name="CardCode" value={this.state.CardCode} state={alerts.CardCode} getOptionsApiRoute={"/api/cbo/ocrd/s"} onChange={this.onFieldChange} />
              </div>
              <div className="col-3 pl-1  ">
                <Date label="Data Documento" name="TaxDate" value={this.state.TaxDate} state={alerts.TaxDate} onChange={this.onFieldChange} />
              </div>
            </div>
            <div className="row">
              <div className="col-4 pr-1">
                <TextBox name="NumAtCard" label="Referência" value={this.state.NumAtCard} state={alerts.NumAtCard} onChange={this.onFieldChange} />
              </div>
              <div className="col-8 pl-1">
                <TextBox name="Comments" label="Observações" value={this.state.Comments} state={alerts.Comments} onChange={this.onFieldChange} />
              </div>
            </div>
            <div className="row">
              <div className="col-9 pr-1">
                <ComboBox label="Artigo" name="ItemCode" value={this.state.ItemCode} state={alerts.ItemCode} getOptionsApiRoute={"/api/cbo/oitm/fse"} onChange={this.onFieldChange} />
              </div>
              <div className="col-3 pl-1">
                <TextBoxNumeric
                  valueType="amount"
          ...
          <div className="sappy-action-bar animation-slide-left">
            <Button color={"success"} onClick={this.onAddDespesa} disabled={sappy.getNum(this.state.totalPagar) === 0}>
