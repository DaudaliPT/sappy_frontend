...
class ModalMeiosPagPagamento extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      settings: sappy.getSettings([
        "FIN.PAG.CONTA_ENCCONTAS",
        "FIN.CCD.CAIXA_PASSAGEM",
        "FIN.CC.CAIXA_PRINCIPAL",
        "FIN.CC.MULTIBANCO",
        "FIN.CC.CHEQUES",
        "FIN.PAG.ART_DEBITOS",
        "FIN.PAG.SERIE_DEBITOS"
      ])
    });
  ...
  onCreatePayment() {
    ...
    let invokeAddDocAPI = () => {
      ...
      let data = {
        ...
        TransferAccountEC: this.state.settings["FIN.PAG.CONTA_ENCCONTAS"],
        ArtigoDebitos: this.state.settings["FIN.PAG.ART_DEBITOS"],
        SerieDebitos: this.state.settings["FIN.PAG.SERIE_DEBITOS"],
        CardCodeEC: this.props.selectedPNcodeCli,
        CodigoClienteDebito: this.props.selectedPNcodeCli,
      ...
      let docs = [...this.props.selectedDocs];
      // Marcar os documentos de cliente para ECC (encontro contas cliente)
      let valorEC = 0;
      let temEC = false;
      docs = docs.map(doc => {
        doc.valorECC = 0;
        if (doc.CardType !== "C") return doc;
        temEC = true;
        doc.valorECC = sappy.getNum(doc.LIQBALANCE);
        valorEC += doc.valorECC;
        return doc;
      });

      if (valorEC < 0) {
        return sappy.showError(null, "O encontro de contas com o cliente não pode ser negativo.");
      }
      if (temEC && valorEC === 0) {
        return sappy.showError(null, "O encontro de contas com o cliente não pode ser zero.");
      }
      // Marcar os documentos de cliente para ECF (encontro contas fornecedor)
      docs = docs.map(doc => {
        let LIQBALANCE = sappy.getNum(doc.LIQBALANCE);
        doc.valorECF = 0;
        if (!(doc.CardType === "S" && LIQBALANCE < 0 && valorEC > 0)) return doc;

        // Controlar o que falta para o encontro de contas
        if (valorEC + LIQBALANCE < 0) LIQBALANCE = -valorEC;

        doc.valorECF = -1 * LIQBALANCE;
        valorEC += LIQBALANCE;
        return doc;
      });

      // Marcar os documentos para pagamento a deduzir
      let TOTALPAGAR = sappy.getNum(this.state.totalPagar);
      docs = docs.map(doc => {
        let LIQBALANCE = sappy.getNum(doc.LIQBALANCE);
        doc.valorDeduzir = 0;
        if (!(doc.CardType === "S" && LIQBALANCE > 0)) return doc;
        doc.valorDeduzir = LIQBALANCE;
        TOTALPAGAR += doc.valorDeduzir;
        return doc;
      });

      // Marcar os documentos para pagamento
      docs = docs.map(doc => {
        let LIQBALANCE = sappy.getNum(doc.LIQBALANCE) + sappy.getNum(doc.valorECF);
        doc.valorPagar = 0;
        if (!(doc.CardType === "S" && LIQBALANCE < 0 && TOTALPAGAR > 0)) return doc;

        // Controlar o que falta para o valor a pagar
        if (TOTALPAGAR + LIQBALANCE < 0) LIQBALANCE = -TOTALPAGAR;

        doc.valorPagar = -1 * LIQBALANCE;
        TOTALPAGAR += LIQBALANCE;
        return doc;
      });

      // adicionar os documentos
      data.PaymentInvoices = docs.map(doc => {
        // proporção que está a ser paga
        let LIQBALANCE = sappy.getNum(doc.LIQBALANCE);
        let bs = Math.abs(sappy.getNum(doc.BaseSum));
        let p = sappy.parseUserDisc(doc.UDISC);

        let TotalDiscount = sappy.round(bs * p.DiscountPercent / 100, 2) + p.DiscountVal;

        let proporcaoECF = sappy.getNum(doc.valorECF) / Math.abs(LIQBALANCE);
        let valorDescontoECF = sappy.round(proporcaoECF * TotalDiscount, 2);

        let proporcaoPagar = sappy.getNum(doc.valorPagar) / Math.abs(LIQBALANCE);
        let valorDescontoPagar = sappy.round(proporcaoPagar * TotalDiscount, 2);
        if (doc.valorECF + doc.valorPagar === -1 * LIQBALANCE) {
          // Garantir que não há diferencas de arredondamentos, quando paga todo o doc
          valorDescontoPagar = sappy.round(TotalDiscount - valorDescontoECF, 2);
        }

        let deb = sappy.parseUserDisc(doc.UDEBITO);
        let valueToUse = sappy.getNum(doc.valorECF) + sappy.getNum(doc.valorPagar);
        let proporcaoDebito = Math.abs(valueToUse / LIQBALANCE);
        let valorDebito = sappy.round(proporcaoDebito * (sappy.round(bs * deb.DiscountPercent / 100, 2) + deb.DiscountVal), 2);

        let inv = {
          BaseRef: doc.BaseRef,
          CardType: doc.CardType,
          Ref2: doc.Ref2,
          InvoiceType: doc.TransType,
          CONTRATO_DESC: doc.CONTRATO_DESC,
      ...
      axios
        .post(`/api/caixa/pagar/payment`, data)
        .then(result => {
