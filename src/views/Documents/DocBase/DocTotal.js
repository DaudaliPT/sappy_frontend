  ...
  render() {
    ...
    return (
      <div id="docTotal" className="container">
        ...
        {sappy.getNum(totals.taxesUnderVat) !== 0 &&
          <div className="row">
            <div className="col">Taxas (Suj.IVA)</div>
            <div className="col">
              <span className="float-right">
                {sappy.format.amount(totals.taxesUnderVat)}
              </span>
            </div>
          </div>}
        <div className="row">
        ...
        {!editable &&
          sappy.getNum(docData.EXTRADISC) !== 0 &&
          <div className="row">
            <div className="col-8">
              Desconto{" (" + sappy.format.percent(sappy.getNum(docData.EXTRADISCPERC)) + ")"}
            </div>
            <div className="col">
              <span className="float-right">
                {sappy.format.amount(sappy.getNum(docData.EXTRADISC))}
              </span>
            </div>
          </div>}
        {editable &&
        ...
        <div className="row">
          ...
          <div className="col">
            <span className="float-right">
              {changing ? "" : sappy.format.amount(totals.vatAmount)}
            </span>
          </div>
        </div>

        {!editable &&
          sappy.getNum(docData.ROUNDVAL) !== 0 &&
          <div className="row">
            <div className="col-6">Arredondamento</div>
            <div className="col">
              <span className="float-right">
                {sappy.format.amount(sappy.getNum(docData.ROUNDVAL))}
              </span>
            </div>
          </div>}
        {editable &&
          <div className="row">
            <div className="col-6">Arredondamento</div>
            <div className="col-6">
              <TextBoxNumeric name="ROUNDVAL" disabled={!editable || changing} value={changing ? "" : sappy.getNum(docData.ROUNDVAL)} onChange={this.props.onFieldChange} valueType="amount" />
            </div>
          </div>}
        <hr />
        ...
        <div className="row total changed">
          ...
          <div className="col-8">
            ...
            {editable && <TextBoxNumeric name="DOCTOTAL" disabled={!editable || changing} value={changing ? "" : totals.totalAmount} onChange={this.props.onFieldChange} valueType="amount" />}
          </div>
        </div>
      </div>
    );
