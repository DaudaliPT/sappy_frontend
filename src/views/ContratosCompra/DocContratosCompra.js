...
class EditModal extends Component {
  ...
  saveToDatabase(stateValues) {
    ...
    axios.post(`/api/contratos/doc/`, data).then(this.loadDocToState).catch(error => sappy.showError(error, "Não foi possivel gravar o contrato"));
  }
  ...
  handleCreateDocument() {
    ...
    let postConfirm = () => axios.post(`/api/contratos/doc/${this.state.ID}/confirm`).then(this.loadDocToState).catch(error => sappy.showError(error, "Não foi possivel criar o contrato"));
  ...
  render() {
    ...
    let strNumero = (this.state.NUMERO && this.state.CARDCODE + "-" + sappy.padZeros(this.state.NUMERO, 3)) || (this.state.ID && "Rascunho") || "Novo";
