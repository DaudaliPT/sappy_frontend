      ...
      <div style={{ textAlign: "right" }} id={"upc" + row.LINENUM}
        ...
        onMouseEnter={e => {
          ...
          that.hoverTimeOutHandle = setTimeout(function () {
            ...
            that.hoverServerRequest = axios({ method: "get", url: `api/inv/oitm/info/${row.ITEM_CODE}/upc` })
              .then(result => {
                let content = []
...
class DocAtualizacaoPrecos extends Component {
  ...
  onMoveTo(nextORprevious) {
    this.serverRequest = axios({
      method: "get",
      url: `/api/inv/precos/doc/${this.state.docData.ID}/${nextORprevious}`
    })
      .then(result => {
        if (result.data)
  ...
  loadDoc() {
    ...
    if (locationState.id) {
      this.serverRequest = axios
        .get(`/api/inv/precos/doc/${locationState.id}`)
        .then(function (result) {
  ...
  createDocLines(itemCodes) {
    let that = this;
    this.serverRequest = axios
      .post(`/api/inv/precos/doc/${this.state.docData.ID}/lines`, { itemCodes })
      .then(function (result) {
        that.setState({
          rows: result.data,
          searchText: "",
  ...
  handleGridRowsUpdated({ fromRow, toRow, updated }) {
    ...
    for (var index = fromRow; index <= toRow; index++) {
      ...
      this.serverRequest = axios
        .patch(`/api/inv/precos/doc/${this.state.docData.ID}/line/${currentRow.LINENUM}`, { ...updated })
        .then(function (result) {
                ...
                <ModalMessageConfirm
                  toggleModal={result => {
                    ...
                    if (result === "CONFIRMADO") {
                      this.serverRequest = axios
                        .delete(`/api/inv/precos/doc/${this.state.docData.ID}`)
                        .then(function (result) {
                          hashHistory.goBack();
                        })
              ...
              currentModal: (
                <ModalMessageConfirm
                  toggleModal={result => {
                    ...
                    if (result === "CONFIRMADO") {
                      this.serverRequest = axios
                        .post(`/api/inv/precos/doc/${this.state.docData.ID}/deletelines`, {
                          Lines: LINENUMS
                        })
                        .then(function (result) {
                          let rows = that.getRows();
              ...
              currentModal: (
                <ModalConfirmDoc temUpdates={temUpdates}
                  ...
                  toggleModal={(result, modalState) => {
                    ...
                    if (result === "CONFIRMADO") {
                      this.serverRequest = axios
                        .post(`/api/inv/precos/doc/${this.state.docData.ID}/confirm`, {
